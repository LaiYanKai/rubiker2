...
//    tacho(1);
//    state_y = digitalRead(PIN_Y);
}

void handler_b(void)
{
    auto y = digitalRead(PIN_Y);
    if (y) {
//      if (prev_state == 1)
//        deg -= 3;
//      else
...
int main (int argc, char **argv)
{
    ...
    state_y = digitalRead(PIN_Y);
    state_b = digitalRead(PIN_B);
    if (wiringPiISR(PIN_Y, INT_EDGE_FALLING, handler_y)  < 0) { ROS_INFO("loser1"); return 1; }
    if (wiringPiISR(PIN_B, INT_EDGE_FALLING, handler_b)  < 0) { ROS_INFO("loser1"); return 1; }
